      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mt-8 bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Projects by Company</h3>
              <p className="text-sm text-gray-500">{selectedYear} - {companyProjectStats.reduce((s, c) => s + c.count, 0)} total projects</p>
            </div>
          </div>
          {companyProjectStats.length > 0 ? (
            <div className="p-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-6">
                {companyProjectStats.map((c, index) => {
                  ...
                  return (
                    <div key={c.name} className={`rounded-lg border p-4 text-center ${colors[index % colors.length]}`}>
                      <p className="text-2xl font-bold">{c.count}</p>
                      <p className="text-sm font-medium mt-1">{c.name}</p>
                      <p className="text-xs font-semibold mt-2 opacity-80">{c.revenue.toLocaleString('en-IE', { style: 'currency', currency: 'EUR', minimumFractionDigits: 0, maximumFractionDigits: 0 })}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="px-6 py-8 text-center text-gray-500">
              <p>No company data available for {selectedYear}</p>
            </div>
          )}
        </div>
